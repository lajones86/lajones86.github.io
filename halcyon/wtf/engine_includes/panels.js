...
const framework_baseline_properties = ["_panel_name", "_panel_object"];
const panel_baseline_properties = ["_panel_display"];
...
class Panel {
	constructor(incoming_panel_data, target, is_update) {
		...
		for (let fbp in framework_baseline_properties) {
			fbp = framework_baseline_properties[fbp];
			if (!this[fbp]) { console.log(this); bad_message(`${fbp} missing`) };
		}
		
		let po = this._panel_object;
		for (let pbp in panel_baseline_properties){
			pbp = panel_baseline_properties[pbp];
			if (!po[pbp]) { console.log(po); bad_message(`${pbp} missing`) }
			else { var panel_data = po[pbp]; };
		}
		
		this.title = make_title_from_varname(this._panel_name);
...
function global_janitor(changed_item) {
	...
	build_panel(starting_panel_object, starting_panel_object.get_dom_object(), true);
}
...
export function build_modules(modules, target) {
	console.log("panels.js received build_modules.js request");
